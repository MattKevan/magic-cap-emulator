// FramebufferPresenter.swift — Metal framebuffer presenter: 480x320 2bpp
// guest buffer rendered aspect-fit, paced by the MTKView.
//
// Frame path: each draw reads the ABI pointer via coreFramebuffer(of:).
...
public final class FramebufferPresenter: NSObject, MTKViewDelegate {
    ...
    /// The view's draw loop is the pacing: `draw(in:)` returns immediately
    /// when the guest frame revision has not changed.
    public func setPaused(_ paused: Bool) { view?.isPaused = paused }

    /// Adopt `view` as the render target and build the persistent resources.
    ...
    public func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) { lastRevision = 0 }
