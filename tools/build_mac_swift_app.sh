...
#
# Deliberately arm64-only and Debug by default: this is a local developer
# staging step, so it builds for the host architecture only and reuses the
# default DerivedData (no -derivedDataPath) to stay incremental on the large
# macOS core. Set CONFIG=Release for a release build.
set -euo pipefail
